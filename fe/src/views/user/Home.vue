<template>
  ...
  >
    <div class="w-full max-h-[250px] overflow-hidden relative" v-if="!showInfo">
      <img src="../../assets/images/home-top-bg.jpg" alt="" />
      <div class="absolute bottom-10 w-full px-5">
        <p class="text-white text-sm font-poppins">Looking for a place?</p>
        <input
          v-model="searchValue"
          @input="onSearch"
          type="text"
          class="form-control block w-full h-9 mt-2 text-base font-poppins text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
          placeholder="Search"
        />
      </div>
    </div>
    ...
    <div
      v-if="!pageLoad && searchValue == '' && !showInfo && trending.length > 0"
      class="p-5 border-b-2"
    >
      <p class="font-poppins text-lg font-semibold">Trending Now!</p>
      <div
        class="flex flex-row gap-x-3 mt-3"
        v-for="trend in trending"
        @click="addViews(trend.id)"
      >
        <img :src="trend.images[0].file_path" width="80" class="rounded-xl" />
        <div class="w-full">
          <p class="font-poppins text-sm font-semibold">
            {{ trend.store_name }}
          </p>
          <div class="flex justify-between w-full">
            <div>
              <vue3-star-ratings
                ...
                :disableClick="true"
                :showControl="false"
                v-model="trend.rating"
              />
              <p class="text-sm font-medium text-gray-400">
                PHP {{ trend.average_cost }}
              </p>
              <p class="text-sm font-medium text-gray-400">
                {{ trend.address }}
              </p>
            ...
            <div>
              <p class="text-sm font-medium text-right text-blue-400">
                {{ trend.category }}
              </p>
            </div>
          </div>
    ...
    <div v-if="!pageLoad" class="p-5 border-b-2">
      <p class="font-poppins text-lg" v-if="recomm.length <= 0">
        Nothing's here...
      </p>
      <p
        class="font-poppins text-lg font-semibold"
        v-if="!pageLoad && searchValue == '' && !showInfo && recomm.length > 0"
      >
        Recommended For You!
      </p>
      <p class="font-poppins text-lg font-semibold" v-if="searchValue != ''">
        Search for '{{ searchValue }}'
      </p>
      <div
        v-if="!pageLoad && !showInfo && recomm.length > 0"
        class="flex flex-row gap-x-3 mt-3"
        v-for="(rec, k) in recomm"
        :key="k"
        @click="addViews(rec.id)"
      >
        <img :src="rec.images[0].file_path" width="80" class="rounded-xl" />
        <div class="w-full">
          <p class="font-poppins text-sm font-semibold">
            {{ rec.store_name }}
          </p>
    ...
    <div v-if="!pageLoad && showInfo && !showFoodInfo" class="px-7 pb-10 pt-16">
      <div
        class="w-full overflow-hidden text-center py-5 bg-yellow-300 absolute left-0 top-0"
      >
        <svg
          v-if="showInfo"
          @click="returnBack"
        ...
        <p class="font-poppins text-lg font-semibold text-green-500">
          KainSaan
        </p>
      </div>
...
async function trends() {
  const res = await store.dispatch("auth/allTrending");
...
async function getRecommended() {
  const res = await store.dispatch("auth/allRecommended", {
    page: page.value,
    keyword: searchValue.value,
  });
  recomm.value.push(...res.data.data);
...
async function submitReview(id) {
  ...
  res.status == 200
    ? (toast.success("Review submitted"), await showFoodEstablishment(id))
    : toast.error("Sorry we cannot add your review");
  review.value = {
    review: "",
    rating: 0,
  };
